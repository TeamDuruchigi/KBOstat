function if_select_by_search(input_name){
    if(player_data)
    {
    ...
    switch (player_data[searched_player][0])
    {
        case "SSG":
        {
    ...
    document.getElementById("player_image").alt = player_data[searched_player][0] + " " + player_data[searched_player][1] + "번 " + player_data[searched_player][2];

    var position = player_data[searched_player][4];
    switch(position)
    {
        case "1B":
        {
    ...
    document.getElementById("player_stat1").innerHTML = player_data[searched_player][7];
    document.getElementById("player_stat2").innerHTML = player_data[searched_player][8];
    document.getElementById("player_stat3").innerHTML = player_data[searched_player][9];
    document.getElementById("player_stat4").innerHTML = player_data[searched_player][10];
    document.getElementById("player_stat5").innerHTML = player_data[searched_player][11];
    ...
    for(let i =0; i<5; i++) {
        for (let j = 0; j < 5; j++) {
            ...
            // 테스트케이스 인풋
            if (player_data[searched_player][6][i][j] < 0.05) {
                back_color = "#900000";
            } else if (player_data[searched_player][6][i][j] < 0.10) {
                back_color = "#851000";
            } else if (player_data[searched_player][6][i][j] < 0.15) {
                back_color = "#802000";
            } else if (player_data[searched_player][6][i][j] < 0.20) {
                back_color = "#753000";
            } else if (player_data[searched_player][6][i][j] < 0.25) {
                back_color = "#704000";
            } else if (player_data[searched_player][6][i][j] < 0.30) {
                back_color = "#655000";
            } else if (player_data[searched_player][6][i][j] < 0.35) {
                back_color = "#606000";
            } else if (player_data[searched_player][6][i][j] < 0.40) {
                back_color = "#557000";
            } else if (player_data[searched_player][6][i][j] < 0.45) {
                back_color = "#208000";
            } else if (0.45 < player_data[searched_player][6][i][j]) {
                back_color = "#009000";
            }
            // 색상 배치
            ...
            strike_zone += "<div class = 'strike_zone', id = 'zone" + (i * 5 + j) + "', " +
                ...
                "<font color='black'>" + player_data[searched_player][6][i][j] + "</font></div>";
            // 클래스 이름 strike_zone
            // id 이름 zone0~zone24
        }
        strike_zone += "</br>";
